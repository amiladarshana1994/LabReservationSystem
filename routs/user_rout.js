...
router.post("/register", function(req,res){
    ...
    user_model_schema.findUserByEmail(req.body.email,function(err,user){
        if (err) throw err;
        if(user){
            res.json({state : false , msg : "There is already user in this email.Please add another email to register."});
            return false ;
        }
        else{
            ...
            user_model_schema.saveUser(new_user,function(err,user){
                if(err){
                    res.json({state : false , msg : "Something Wrong..! Please try again..."});
                    return false ;
                }
                else{
                    res.json({state : true , msg : "Register Success"});
                }
            }); 
        }
...
router.get('/home',passport.authenticate('jwt', { session: false }),function(req, res) {
    ...
    reservation_model_schema.findUserReservation(regno ,function(err,reservations){
        //console.log(reservations);
        if(err){
            res.json({state : false , msg : "Something went Wrong.. Please Try Again.."});
        }
